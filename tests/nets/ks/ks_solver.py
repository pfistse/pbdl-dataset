...
class DifferentiableKS():
    def __init__(self, resolution, dt):
        ...
        self.dt = dt
        self.doublestep = 1 # increase difficulty for correction tasks
...
# # older, single step physics loss
# def unsupervised_loss_org(prediction, diff_ks):
...
#     L = loss(prediction[:,1:,:].reshape((-1,prediction.shape[-1])) , stepped_prediction.native(['i','x']) )
#     # L = torch.mean( ( prediction[:,1:,:].reshape((-1,prediction.shape[-1]))-
#     #                      stepped_prediction.native(['i','x']) ) **2 )
#     # L = torch.mean( ( prediction[:,1:,:].reshape((-1,prediction.shape[-1]))-
#     #                      stepped_prediction.native(['i','x']) ) **2/
#     #                    (2.+torch.mean(torch.abs(prediction[:,1:,:]))))
#     return L
...
# run via "device_iter" 
# unlike previous version, doesnt use batches... (slower) still used in train script
